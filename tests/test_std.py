...
from json_syntax.helpers import J2P, P2J, NoneType
...
def test_atoms_str():
    ...
    inspect = std.atoms(verb=IP, type=str, ctx=Fail())
    assert inspect("string")
    assert not inspect(5)

    inspect = std.atoms(verb=IJ, type=str, ctx=Fail())
    assert inspect("string")
    assert not inspect(5)
...
def test_atoms_int():
    ...
    inspect = std.atoms(verb=IP, type=int, ctx=Fail())
    assert not inspect("string")
    assert inspect(5)

    inspect = std.atoms(verb=IJ, type=int, ctx=Fail())
    assert not inspect("string")
    assert inspect(5)
...
def test_atoms_bool():
    ...
    inspect = std.atoms(verb=IP, type=bool, ctx=Fail())
    assert not inspect("string")
    assert inspect(True)

    inspect = std.atoms(verb=IJ, type=bool, ctx=Fail())
    assert not inspect("string")
...
def test_iso_dates_picklable():
    ...
    actions = [std.iso_dates(verb=verb, typ=typ, ctx=Fail()) for verb in [J2P, P2J] for typ in [date, datetime]]
    assert None not in actions
    dumps(actions)
...
def test_enums_picklable():
    ...
    actions = [std.enums(verb=verb, typ=typ, ctx=Fail()) for verb in [J2P, P2J] for typ in [Enum1, Enum2]]
    assert None not in actions
    dumps(actions)
...
def test_list_or_tuple_disregards():
    "Test that list_or_tuple disregards unknown types and verbs."

    assert std.list_or_tuple(verb="unknown", typ=List[int], ctx=Fail()) is None
    assert std.list_or_tuple(verb="unknown", typ=Tuple[int, ...], ctx=Fail()) is None
    assert std.list_or_tuple(verb=P2J, typ=bool, ctx=Fail()) is None
    assert std.list_or_tuple(verb=J2P, typ=Tuple[int, str], ctx=Fail()) is None


def test_list_or_tuple_lists():
    "Test that list_or_tuple will generate encoders and decoders for lists."

    ctx = Mock(lookup_inner=Mock(return_value=str))

    encoder = std.list_or_tuple(verb=P2J, typ=List[str], ctx=ctx)
    assert encoder([33, 77]) == ["33", "77"]

    decoder = std.list_or_tuple(verb=J2P, typ=List[str], ctx=ctx)
    assert decoder([33, 77]) == ["33", "77"]


def test_list_or_tuple_tuples():
    "Test that list_or_tuple will generate encoders and decoders for homogenous tuples."

    ctx = Mock(lookup_inner=Mock(return_value=str))

    encoder = std.list_or_tuple(verb=P2J, typ=Tuple[str, ...], ctx=ctx)
    assert encoder((33, 77)) == ["33", "77"]

    decoder = std.list_or_tuple(verb=J2P, typ=Tuple[str, ...], ctx=ctx)
    assert decoder([33, 77]) == ("33", "77")
...
def test_set_or_frozenset_disregards():
    "Test that set_or_frozenset disregards unknown types and verbs."

    assert std.set_or_frozenset(verb="unknown", typ=Set[int], ctx=Fail()) is None
    assert std.set_or_frozenset(verb="unknown", typ=FrozenSet[set], ctx=Fail()) is None
    assert std.set_or_frozenset(verb=P2J, typ=bool, ctx=Fail()) is None
    assert std.set_or_frozenset(verb=J2P, typ=List[str], ctx=Fail()) is None


def test_set_or_frozenset_sets():
    "Test that set_or_frozenset will generate encoders and decoders for sets."

    ctx = Mock(lookup_inner=Mock(return_value=str))

    encoder = std.set_or_frozenset(verb=P2J, typ=Set[str], ctx=ctx)
    actual = encoder({1, 2, 2, 3})
    actual.sort()
    assert actual == ["1", "2", "3"]

    decoder = std.set_or_frozenset(verb=J2P, typ=Set[str], ctx=ctx)
    assert decoder([1, 2, 2, 3]) == {"1", "2", "3"}


def test_set_or_frozenset_frozen():
    "Test that set_or_frozenset will generate encoders and decoders for frozen sets."

    ctx = Mock(lookup_inner=Mock(return_value=str))

    encoder = std.set_or_frozenset(verb=P2J, typ=FrozenSet[str], ctx=ctx)
    actual = encoder(frozenset([1, 2, 2, 3]))
    actual.sort()
    assert actual == ["1", "2", "3"]

    decoder = std.set_or_frozenset(verb=J2P, typ=FrozenSet[str], ctx=ctx)
