...
If you're like the authors, you tried writing a encoding function that attempted to encode and decode by interrogating the types at runtime, maybe calling some method like `asdict`. This works fine for generating JSON, but it gets sketchy when trying to decode the same JSON.
...
At the time of writing, the library is in **alpha** and the API may move around or be renamed.
 ...
 * Collections including `typing.List[E]`, `typing.Tuple[E, ...]`, `typing.Set[E]` and `typing.FrozenSet[E]`.
    * The ellipsis is part of the syntax! It indicates a homogenous tuple, essentially a frozen list.
...
Generally, the `typing` module simple provides capital letter type names that obviously correspond to the internal types.
...
Tuple is a special case. In Python, they're often used to mean "frozenlist", so `Tuple[E, ...]` (the `...` is the actual syntax!) indicates all elements have the type `E`.

They're also used to represent an unnamed record. In this case, you can use `Tuple[A, B, C, D]` or however many types. But don't do that, just make a `dataclass`.
...
A union type lets you present alternate types that the converters will attempt in sequence:

```
typing.Union[MyType, int, MyEnum]
```

The `unions` rule is not a discriminated union; it won't add any additional information to the value or wrap it in a flag. Thus, ambiguous values, especially JSON representations, may confuse the decoder.
...
_Encoders and decoders do very little checking._ Especially, if you're translating Python to JSON, it's assumed that
your Python classes are correct. The encoders and decoders may mask subtle issues as they are calling constructors
like `str` and `int` for you. And, by design, if you're translating from JSON to Python, it's assumed you want to be
tolerant of extra data.

_Everything to do with typing._ It's a bit magical and sort of wasn't designed for this.

_Union types._ You can use `typing.Union` to allow a member to be one of some number of alternates, but there are some caveats. These are documented in code in `test_unions`, but in plain English:
 ...
 * `Union[str, Stringly]` will never construct an instance that is represented as a string in JSON.
    * This includes enums, dates and special float values (`Nan`, `-inf`, etc.) may be represented as strings.
 * `Union[datetime, date]` will never construct a date because `YYYY-MM-DD` is a valid datetime according to ISO8601.
 * `Union[Dict[str, Value], MyAttrs]` will never construct `MyAttrs` if all its attributes are `Value`.
...
_Numbers are hard._ See the rules named `floats`, `floats_nan_str`, `decimals`, `decimals_as_str` for details on how to get numbers to transmit reliably.
